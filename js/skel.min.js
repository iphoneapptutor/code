	...
	var a = {
		...
		detachAllElements : function() {
			...
			if ( b = a.cache.elements[d].object, b.parentNode && (!b.parentNode || b.parentNode.tagName))
				if (b.parentNode.removeChild(b), a.cache.elements[d].onDetach)
		...
		attachElements : function(d) {
			...
			if (0 != b[f].length)
				...
				if ( d = a.locations[b[f][e].location]) {
					if (d.appendChild(b[f][e].object), b[f][e].onAttach)
						b[f][e].onAttach()
				} else
					c.push(b[f][e]);
			0 < c.length && a.DOMReady(function() {
				for (var b in c)
				if (a.locations[c[b].location].appendChild(c[b].object), c[b].onAttach)
					c[b].onAttach()
			})
		},
		poll : function() {
		...
		updateState : function() {
			var d, b, c, f = [], e = a.stateId.substring(1).split("#");
			for (b in e)
		...
		changeState : function(d) {
			...
			if (a.cache.states[a.stateId])
				...
				for (b in d)
				a.extend(e.config, a.breakpoints[d[b]].config);
				if (a.config.boxModel) {
					if (!( c = a.getCachedElement("iBM")))
						c = a.cacheElement("iBM", a.newInline("*,*:before,*:after{-moz-@;-webkit-@;-o-@;-ms-@;@}".replace(/@/g, "box-sizing:" + a.config.boxModel + "-box")), "head", 3);
					e.elements.push(c)
				}
				if (a.config.resetCSS) {
					if (!( c = a.getCachedElement("iR")))
						c = a.cacheElement("iR", a.newInline(a.css.r), "head", 2);
					e.elements.push(c)
				} else if (a.config.normalizeCSS) {
					if (!( c = a.getCachedElement("iN")))
						c = a.cacheElement("iN", a.newInline(a.css.n), "head", 2);
					e.elements.push(c)
				}
				if (a.config.prefix) {
					if (!( c = a.getCachedElement("ssB")))
						c = a.cacheElement("ssB", a.newStyleSheet(a.config.prefix + ".css"), "head", 4);
					e.elements.push(c)
				}
				if (e.config.lockViewport) {
					if (!( c = a.getCachedElement("mV")))
						c = a.cacheElement("mV", a.newMeta("viewport", "width=device-width,initial-scale=1.0,minimum-scale=1.0,maximum-scale=1.0,user-scalable=no"), "head", 1);
					e.elements.push(c)
				}
				...
				0 == f && ( f = 960, u = "px");
				if (!( c = a.getCachedElement("iC" + f + u)))
					c = a.cacheElement("iC" + f + u, a.newInline("." + (a.config.noConflict ? a.config.noConflictPrefix + "-" : "") + "container{width:" + f + u + " !important;margin: 0 auto;}"), "head", 3);
				e.elements.push(c);
				switch(e.config.grid.gutters) {
					case 0:
						if (!( c = a.getCachedElement("iG0")))
							c = a.cacheElement("iG0", a.newInline(a.css.g0 + a.css.gF), "head", 3);
						e.elements.push(c);
						break;
					case 1:
						if (!( c = a.getCachedElement("iG1")))
							c = a.cacheElement("iG1", a.newInline(a.css.g1 + a.css.gF), "head", 3);
						e.elements.push(c);
						break;
					case 4:
						if (!( c = a.getCachedElement("iG4")))
							c = a.cacheElement("iG4", a.newInline(a.css.g4 + a.css.gF), "head", 3);
						e.elements.push(c);
						break;
					case 6:
						if (!( c = a.getCachedElement("iG6")))
							c = a.cacheElement("iG6", a.newInline(a.css.g6 + a.css.gF), "head", 3);
						e.elements.push(c);
						break;
					default:
						if (!( c = a.getCachedElement("iG2")))
							c = a.cacheElement("iG2", a.newInline(a.css.g2 + a.css.gF), "head", 3);
						e.elements.push(c)
				}
				if (e.config.grid.collapse) {
					if (!( c = a.getCachedElement("iGCo")))
						c = a.cacheElement("iGCo", a.newInline(a.css.gR + a.css.gCo), "head", 3)
				} else if (!( c = a.getCachedElement("iGNoCo")))
					c = a.cacheElement("iGNoCo", a.newInline(a.css.gR), "head", 3);
				e.elements.push(c);
				...
				for (b in d) {
					if (a.breakpoints[d[b]].config.hasStyleSheet && a.config.prefix) {
						if (!( c = a.getCachedElement("ss" + d[b])))
							c = a.cacheElement("ss" + d[b], a.newStyleSheet(a.config.prefix + "-" + d[b] + ".css"), "head", 5);
						e.elements.push(c)
					}
					if (0 < a.breakpoints[d[b]].elements.length)
						for (c in a.breakpoints[d[b]].elements)
						e.elements.push(a.breakpoints[d[b]].elements[c])
				}
			}
			a.detachAllElements();
			...
			a.DOMReady(function() {
				...
				if (( a = document.getElementsByClassName("skel-cell-mainContent")) && 0 < a.length)
					if ( a = a[0], e.config.grid.collapse)
						b = document.createElement("div"), b.innerHTML = "", b.id = "skel-cell-mainContent-placeholder", a.parentNode.insertBefore(b, a.nextSibling), a.parentNode.insertBefore(a, a.parentNode.firstChild);
					else if ( b = document.getElementById("skel-cell-mainContent-placeholder"))
						a.parentNode.insertBefore(a, b), a.parentNode.removeChild(b)
			});
		...
		initConfig : function() {
			...
			for (k in a.config.events)
			a.bind(k, a.config.events[k]);
		...
		init : function() {
			...
			(function() {
				var b = window, c = function(a) {
					...
					a = !1;
					if (!e)
						if ( e = !0, "loading" !== d.readyState && l(), d.addEventListener)
							d.addEventListener("DOMContentLoaded", h, !1), b.addEventListener("load", h, !1);
			...
			a.indexOf = Array.prototype.indexOf ? function(a, c) {
				...
				var d = a.length >>> 0, e = Number(c) || 0, e = 0 > e ? Math.ceil(e) : Math.floor(e);
				for (0 > e && (e += d); e < d; e++)
					if ( a instanceof Array && e in a && a[e] === c)
						return e;
				return -1
			};
			a.initConfig();
